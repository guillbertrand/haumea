...
def main():
    ...
    if(args.yourprojectname):
        ...
        try:
            ...
            logger.info('Create content folder "%s/content/"' % args.yourprojectname)
            logger.info('Create content layouts "%s/layouts/"' % args.yourprojectname)
            logger.info('Add sample files...')
            logger.info('\U0001F331  Yeah ! your project is ready \U0001F331');
            logger.info('Try to run the command "cd %s && haumea serve" to test it ! \U0001F30D' % args.yourprojectname);
        except:
            logger.error('Unable to create haumea skeleton in "%s" directory' % args.yourprojectname)
